...
df_arm <- lapply(df_arm, function(x){as.factor(x)})

df_rules <- apriori(df_arm, 
                    parameter = list(support = 0.1,
                                     confidence = 0.5))

inspect(head(sort(df_rules, by = "confidence"), 3))

# Convert to a transactional data set
df_arm_transactinoal <- as(df_arm, "transactions")

df_rules <- apriori(df_arm_transactinoal, 
                    parameter = list(support = 0.1,
                                     confidence = 0.5))
```
